...
`UIP-xxxx.md`, where `xxxx` is the number of the UID left-padded with zeroes.
...
Headers containing metadata about the UIP. They MUST be enclosed in triple
backticks to mark them up as code block.

The preamble has the following fields:

* `Author`: Name and email address of author of the UIP
* `Co-Author`: Name and email address of additional authors of the UIP (field
...
`Superseded` and `Superseded-by` MUST be added when UID has status `Replaced`.
...
before making the UIP final.

At the moment of the creation of the pull request the UIP is considered created
and under evaluation. At the moment of  merging the pull request the UIP will be
considered final. The `status` inside the document should reflect those changes.

In the case of a new UIP superseding a document, the pull request for the new
UIP should also take care of changing the status of the document to superseded.

#### UIP life cycle

The author is responsible for driving the UIP and establishing rough consensus
about the proposal. The status of the UIP reflects the different states the
proposal goes through during this process.
...
changed to `Replaced` and the UIPs which supersede it should be noted in the
